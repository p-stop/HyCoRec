...
class HyCoRecModel(BaseModel):
    ...
    def encode_user_repr_with_weight_fn(self, related_items, related_entities, related_words,
        ...
        # Item 超图
        item_embedding = torch.zeros((1, self.kg_emb_dim), device=self.device) #initialize hg embedding[1 * kg_emb_dim]
        if len(related_items) > 0:
            items, item_hyper_edge_index = self._get_hypergraph(related_items, self.item_adj)
            ...
            )#extract embeddibngs only connected to current data
            
            hyperedge_weight = None#*# think edge_weight build
            if item_weight_fn is not None:
                # 调用权重生成函数
                node_prob, node_weight = item_weight_fn(sub_item_embedding, sub_item_edge_index)
                # 将边权重聚合为超边权重（mean pooling）
                hyperedge_weight = self._edge_weight_to_hyperedge_weight(edge_prob, sub_item_edge_index)
                weight_info['item'] = {'weight': edge_prob, 'logits': weight_logits}
            
            item_embedding = self.hyper_conv_item(sub_item_embedding, sub_item_edge_index, hyperedge_weight=hyperedge_weight)
